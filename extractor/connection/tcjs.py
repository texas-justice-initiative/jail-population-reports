...
FILENAMES: Dict[str, str] = {'jail_population': 'AbbreRptCurrent.pdf', 'pregnancies': 'PregnantFemaleReportingCurrent.pdf', 'immigration': 'ImmigrationDetainerReportCurrent.pdf'}

# dates are two-digit month and 4-digit years
# URL format: {BASE}/{YEAR}/{MONTH}/{FILENAME}

class TCJSConnection:
    def list_files(file_slug: str, start: datetime, end: datetime) -> List[datetime]:
        pass
    ...
    def download(file_slug: str, data_date: datetime, output: Path) -> None:
        ...
        with open(output_file / f'{file_slug}.pdf', 'wb') as output_file:
            file_contents: requests.Response = requests.get(f'{BASE_URL}/2021/09/{FILENAMES[file_slug]}')
